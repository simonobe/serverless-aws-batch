...
function getBatchJobQueueName() {
  ...
  }-${this.provider.getStage()}-JobQueue`;
}
...
function validateAWSBatchServerlessConfig() {
  ...
  if (!batch.hasOwnProperty("SecurityGroupIds")) {
    throw new Error(
      "'batch' configuration does not contain property 'SecurityGroupIds' (make sure it's capitalized)"
    );
  }
  if (!batch.hasOwnProperty("Subnets")) {
    throw new Error(
      "'batch' configuration does not contain property 'Subnets' (make sure it's capitalized)"
    );
  }
}
...
function generateBatchServiceRole() {
  ...
  return {
    [this.provider.naming.getBatchServiceRoleLogicalId()]: JSON.parse(
      batchServiceRoleTemplate
    )
  };
}
...
function generateBatchInstanceRole() {
  ...
  return {
    [this.provider.naming.getBatchInstanceManagementRoleLogicalId()]: JSON.parse(
      batchInstanceManagementRoleTemplate
    ),
    [this.provider.naming.getBatchInstanceManagementProfileLogicalId()]: JSON.parse(
      batchInstanceProfileTemplate
    )
  };
}
...
function generateBatchSpotFleetRole() {
  ...
  return {
    [this.provider.naming.getBatchSpotFleetManagementRoleLogicalId()]: JSON.parse(
      batchSpotRoleManagementTemplate
    )
  };
}
...
function generateLambdaScheduleExecutionRole() {
  ...
  // Setup the JobQueue to push tasks to
  return {
    [this.provider.naming.getLambdaScheduleExecutionRoleLogicalId()]: JSON.parse(
      lambdaScheduleExecutionRoleTemplate
    )
  };
}
...
function generateBatchJobQueue() {
  ...
  return {
    [this.provider.naming.getBatchJobQueueLogicalId()]: JSON.parse(
      batchJobQueueTemplate
    )
  };
}
...
function generateBatchComputeEnvironment() {
  ...
  const computeEnvironmentName = `${
    this.provider.serverless.service.service
  }-${this.provider.getStage()}-ComputeEnvironment-${Math.floor(
  ...
  return {
    [this.provider.naming.getBatchComputeEnvironmentLogicalId()]: JSON.parse(
      batchComputeEnvironmentTemplate
    )
  };
}
...
function generateAWSBatchTemplate() {
  ...
  const newBatchInstanceManagementRoleObject = generateBatchInstanceRole.bind(
    this
  )();
  const newBatchSpotFleetManagementObject = generateBatchSpotFleetRole.bind(
    this
  )();
  const newLambdaScheduleExecutionRoleObject = generateLambdaScheduleExecutionRole.bind(
    this
  )();
  const newBatchJobQueueObject = generateBatchJobQueue.bind(this)();
  const newBatchComputeEnvironmentObject = generateBatchComputeEnvironment.bind(
    this
  )();
