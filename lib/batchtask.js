...
function getJobDefinitionLogicalId(functionName) {
  let logicalId = `JobDefinition${functionName
    .charAt(0)
...
function getBatchJobExecutionRoleLogicalId(functionName) {
  let logicalId = `IamRoleBatchJobExecution${functionName
    .charAt(0)
    .toUpperCase()}${functionName.slice(1)}`;
  return logicalId.length > 64 ? logicalId.substring(0, 64) : logicalId;
}
...
function compileBatchTask(functionName) {
  ...
  functionObject.getJobDefinitionName = function () {
    ...
    }-${this.provider.getStage()}-${functionName}`;
  }.bind(this);
  ...
  updateContainerProperties.bind(this)(
    newFunction,
    functionObject,
    functionName
  );
  ...
  const newBatchJobExecutionRoleObject = generateBatchJobExecutionRole.bind(
    this
    ...
    [functionName]: {
      ...
      environment: {
        EVENT_LOGGING_ENABLED:
          functionObject.batch.hasOwnProperty('scheduleLoggingEnabled') &&
          functionObject.batch.scheduleLoggingEnabled,
  ...
  _.unset(
    ...
    `${functionName}.iamRoleStatements`
  );
  return BbPromise.resolve();
}
...
function updateContainerProperties(newFunction, functionObject, functionName) {
  ...
  if (!newFunction.Properties.ContainerProperties.hasOwnProperty('Command')) {
    if (!functionObject.handler) {
      throw new this.serverless.classes.Error(
        `Could not find handler for batch task ${functionObject.name}`
      );
    }
  ...
  if (
    !newFunction.Properties.ContainerProperties.hasOwnProperty('JobRoleArn')
  ) {
...
function updateTimeout(newFunction, functionObject) {
  ...
  if (
    !newFunction.Properties.Timeout.hasOwnProperty('AttemptDurationSeconds')
  ) {
...
function getMemorySize(functionObject) {
  return (
    Number(functionObject.batch.memory) || Number(functionObject.memory) || 2048
  );
}
  ...
  return (
    Number(
      functionObject.batch.Timeout
        ? functionObject.batch.Timeout.AttemptDurationSeconds
        : undefined
    ) ||
...
async function generateLambdaScheduleArtifact(functionName) {
  this.serverless.cli.log(
    `Building lambda schedule artifact for: "${functionName}"...`
  );

  const functionArtifactFileName = this.provider.naming.getFunctionArtifactName(
    functionName
  );
...
function generateBatchJobExecutionRole(functionName, functionObject) {
  const batchJobExecutionRoleName = `BatchJobRole-${this.provider.getRegion()}-${this.provider.getStage()}-${
    this.provider.serverless.service.service
  }-${functionName}`.substring(0, 64);
  ...
  return {
    [this.provider.naming.getBatchJobExecutionRoleLogicalId(
      functionName
...
function compileBatchTasks() {
  ...
  return BbPromise.each(allFunctions, (functionName) =>
    compileBatchTask.bind(this)(functionName)
  );
}
